...
object Drawing {
  ...
  def drawGrid(x0: Int, y0: Int, w: Int, h: Int, gridWidth: Int,
    ...
    for (x <- x0 + 1 until x0 + w) {
      val symbol = if (x % gridWidth == 0 && showVertical) Symbols.SH_TO_SD else Symbols.SH
      val symbol2 = if (x % gridWidth == 0 && showVertical) Symbols.SH_TO_SU else Symbols.SH
      screen.put(x, y0, symbol, fg, bg)
      screen.put(x, y0 + h, symbol2, fg, bg)
    }
    for (y <- y0 + 1 until y0 + h) {
      val symbol = if (y % gridHeight == 0 && showHorizontal) Symbols.SV_TO_SR else Symbols.SV
      val symbol2 = if (y % gridHeight == 0 && showHorizontal) Symbols.SV_TO_SL else Symbols.SV
      screen.put(x0, y, symbol, fg, bg)
      screen.put(x0 + w, y, symbol2, fg, bg)
    }
    if (showVertical)
      for (y <- y0 + 1 until y0 + h; x <- x0 + gridWidth until x0 + w by gridWidth)
        screen.put(x, y, Symbols.SV, fg, bg)
    if (showHorizontal)
      for (x <- x0 + 1 until x0 + w; y <- y0 + gridHeight until y0 + h by gridHeight)
